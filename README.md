...
- `[audio].worklet_target_latency_ms`: initial jitter-buffer target for raw and ADPCM AudioWorklet playback. The default is 160 ms.
- `[audio].worklet_high_water_ms`: backlog ceiling that causes stale Worklet audio to be discarded. It must exceed the target; the default is 320 ms.
