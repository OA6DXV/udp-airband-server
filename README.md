...
- `Compressed`: audio generated by `ffmpeg`. Desktop browsers use low-latency Opus/WebM. iPhone and iPad use native HLS/AAC for Safari compatibility.

Desktop browsers default to `Uncompressed`. Mobile browsers default to `Compressed` when the server has `ffmpeg` available; otherwise they fall back to `Uncompressed`.

Install `ffmpeg` on Ubuntu to enable compressed audio:
