...
function loadAdminAuthConfig(env = process.env) {
  const config = {
    ...
    altchaCost: readInteger(env, 'ADMIN_ALTCHA_COST', 1500, 1000, 100000),
    altchaCounterMin: readInteger(env, 'ADMIN_ALTCHA_COUNTER_MIN', 2000, 1000, 100000),
    altchaCounterMax: readInteger(env, 'ADMIN_ALTCHA_COUNTER_MAX', 4000, 1000, 200000),
...
async function upsertAdministrator({ database, password, passwordHasher = createPasswordHasher(), username, now = Date.now() }) {
  ...
  transaction(() => {
    ...
    db.prepare('DELETE FROM admin_sessions WHERE admin_id = 1').run();
    db.prepare(`
      INSERT INTO admin_user_events (admin_id, username, action, created_at)
      VALUES (1, ?, ?, ?)
    `).run(normalizedUsername, existing ? 'password_changed' : 'created', now);
  });
