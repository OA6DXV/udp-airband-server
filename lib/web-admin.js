...
function createWebAdmin(options) {
  ...
  async function handleRequest(req, res) {
    ...
    if (req.method === 'GET' && pathname === '/users') {
      send(res, 200, assets.usersHtml, 'text/html; charset=utf-8', { googleCharts: true });
      return;
    }
    ...
    if (req.method === 'GET' && pathname === '/users.js') {
      send(res, 200, assets.usersJs, 'application/javascript; charset=utf-8');
      return;
    }
  ...
  function send(res, statusCode, body, contentType, securityOptions) {
  ...
  function securityHeaders(options = {}) {
    const contentSecurityPolicy = options.googleCharts
      ? "default-src 'self'; script-src 'self' https://www.gstatic.com https://www.google.com; style-src 'self' 'unsafe-inline' https://www.gstatic.com https://www.google.com; img-src 'self' data: https://www.gstatic.com https://www.google.com; connect-src 'self' https://www.gstatic.com https://www.google.com; frame-ancestors 'none'; base-uri 'none'; form-action 'self'"
