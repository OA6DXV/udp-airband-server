...
function assessPasswordStrength(password, username = '') {
  ...
  if (value.length < 8) warnings.push('is short');
  if (commonPasswords.has(lower)) warnings.push('is a common password');
  if (/^(.)\1+$/.test(value)) warnings.push('uses one repeated character');
  if (/^(?:012345|12345|123456|234567|345678|456789|987654|876543|765432)/.test(value)) {
    warnings.push('uses a simple number sequence');
  }
  if (normalizedUsername && lower.includes(normalizedUsername)) warnings.push('contains the username');
  if (!/[A-Za-z]/.test(value) || !/[^A-Za-z]/.test(value)) warnings.push('uses only one character class');
