...
function createCompressedManager(options) {
  ...
  const encodeBinaryFrame = options.encodeWsBinary || ((frame) => frame);
  const sendEncodedFrame = options.sendWsEncoded || sendWsBinary;

  const ffmpegAvailable = hasFfmpeg(ffmpegPath, spawnSync);
  const available = true;
  const adpcmPacer = createFramePacer({
    frameMs: adpcmFrameMs,
    deliver: deliverAdpcmFrame,
    onDrop(stream, frames) {
      if (debugEnabled) logger.debug('adpcm_pacer_overflow', { stream: stream.name, droppedFrames: frames.length });
    },
  });
  ...
  function serveHttpOpus(stream, requestUrl, res) {
    ...
    const ffmpeg = spawn(ffmpegPath, args, { stdio: ['pipe', 'pipe', shouldCaptureFfmpegStderr(logger) ? 'pipe' : 'ignore'] });
  ...
  function cleanupClient(stream, opusClient, preserveSocket = false) {
    if (!stream.opusClients.has(opusClient)) return;
    stream.opusClients.delete(opusClient);
    if (opusClient.mode === 'adpcm' && !hasAdpcmClients(stream)) {
      adpcmPacer.reset(stream);
    }
    ...
    if (!preserveSocket && opusClient.socket && !opusClient.socket.destroyed) {
  ...
  function releaseWebSocket(stream, socket) {
    for (const opusClient of stream.opusClients) {
      if (opusClient.socket !== socket) continue;
      cleanupClient(stream, opusClient, true);
      return true;
    }
    return false;
  }
  ...
  function hasAdpcmClients(stream) {
    for (const client of stream.opusClients) {
      if (client.mode === 'adpcm') return true;
    }
    return false;
  }
  ...
  function writeStreamInput(stream, buffer) {
    let hasAdpcmClient = false;
    for (const opusClient of stream.opusClients) {
      ...
      if (opusClient.mode === 'adpcm') {
        ...
        writeInput(stream, opusClient, buffer);
      }
    }

    if (hasAdpcmClient) writeAdpcmInput(stream, buffer);
  }

  function writeAdpcmInput(stream, buffer) {
    const frames = encodeAdpcmFrames(stream, buffer, adpcmFrameMs);
    if (adpcmPacing) {
      adpcmPacer.enqueue(stream, frames);
      return;
    }
    for (const frame of frames) deliverAdpcmFrame(stream, frame);
  }

  function deliverAdpcmFrame(stream, frame) {
    const encodedFrame = encodeBinaryFrame(frame);
    for (const opusClient of stream.opusClients) {
      if (opusClient.mode !== 'adpcm') continue;
      if (!isWritableClient(opusClient) || opusClient.backpressured) {
        opusClient.droppedBytes += frame.length;
        if (debugEnabled) logger.debug('adpcm_frame_dropped', { stream: stream.name, client: opusClient.clientId, bytes: frame.length, droppedBytes: opusClient.droppedBytes });
        continue;
      }
      const ok = sendEncodedFrame(opusClient.socket, encodedFrame);
      if (!ok) {
        opusClient.backpressured = true;
