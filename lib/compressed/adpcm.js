...
function encodeAdpcmFrames(stream, input, frameMs) {
  ...
  if (!stream.adpcmLastInputAt || now - stream.adpcmLastInputAt > BURST_RESET_MS) {
    ...
    stream.adpcmPendingBuffer = Buffer.alloc(0);
  }
  stream.adpcmLastInputAt = now;

  const frameBytes = maxFrames * stream.channels * 4;
  const pending = stream.adpcmPendingBuffer || Buffer.alloc(0);
  const available = pending.length === 0 ? input : Buffer.concat([pending, input]);
  const chunks = [];
  let offsetBytes = 0;
  while (available.length - offsetBytes >= frameBytes) {
    const pcmFrame = available.subarray(offsetBytes, offsetBytes + frameBytes);
    chunks.push(encodeAdpcmFrame(stream, pcmFrame, 0, maxFrames));
    offsetBytes += frameBytes;
  }
  stream.adpcmPendingBuffer = Buffer.from(available.subarray(offsetBytes));
  return chunks;
}
