...
function encodeAdpcmFrame(stream, input, offsetFrame, frames) {
  ...
  for (let channel = 0; channel < channels; channel += 1) {
    const predictor = readPcm16(input, offsetFrame, channel, channels);
    states.push({ predictor, index: 0 });
    const stateOffset = 20 + channel * 4;
    output.writeInt16LE(predictor, stateOffset);
    output.writeUInt8(0, stateOffset + 2);
  ...
  for (let frame = 1; frame < frames; frame += 1) {
    for (let channel = 0; channel < channels; channel += 1) {
      const sample = readPcm16(input, offsetFrame + frame, channel, channels);
...
function readPcm16(input, frame, channel, channels) {
  const sample = input.readFloatLE((frame * channels + channel) * 4);
  return clamp(Math.round(Math.max(-1, Math.min(1, sample)) * 32767), -32768, 32767);
}
