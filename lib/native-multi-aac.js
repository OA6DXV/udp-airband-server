...
function createNativeMultiAac(options) {
  ...
  function serve(requestUrl, res, normalizeClientId, remoteAddress = '') {
    ...
    const session = createSession(clientId, selectedStreams, res, remoteAddress);
    sessions.set(clientId, session);
    if (onClientConnected) onClientConnected(clientId, remoteAddress);
    for (const stream of selectedStreams) addListenerMode(stream, clientId, 'native-aac');

    res.on('close', () => cleanupSession(session));
  }

  function createSession(clientId, selectedStreams, res, remoteAddress) {
  ...
  function cleanupSession(session) {
    ...
    if (onClientDisconnected) onClientDisconnected(session.clientId, session.remoteAddress);
