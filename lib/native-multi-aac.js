...
function createNativeMultiAac(options) {
  ...
  function createSession(clientId, selectedStreams, res) {
    ...
    const session = {
      ...
      queues,
      gains,
      streamDelays: new Map(selectedStreams.map((stream) => [stream.name, 0])),
      pendingDelayRawAt: new Map(),
      syncStreams: new Set(),
      lastDelaySyncAt: 0,
      ffmpeg,
      res,
  ...
  function pushPcm(stream, buffer) {
    for (const session of sessions.values()) {
      ...
      if (!queue) continue;
      queue.push({ samples: downmixToMono(buffer, stream.channels), offset: 0, rawAt: stream.lastUdpAt || Date.now() });
  ...
  function writeMixedFrame(session) {
    ...
    const now = Date.now();
    if (now - session.lastDelaySyncAt >= delayResyncMs) {
      requestSessionResync(session);
    }
    ...
    for (let frame = 0; frame < session.frameCount; frame += 1) {
      ...
      for (const stream of session.selectedStreams) {
        const queued = readQueuedSample(session.queues.get(stream.name));
        if (queued.rawAt && session.syncStreams.has(stream.name) && !session.pendingDelayRawAt.has(stream.name)) {
          session.pendingDelayRawAt.set(stream.name, queued.rawAt);
        }
        mixed += queued.value * (session.gains.get(stream.name) || 0);
      }
  ...
  function readQueuedSample(queue) {
    if (!queue || queue.length === 0) return { value: 0, rawAt: 0 };
    const chunk = queue[0];
    const value = chunk.samples[chunk.offset] || 0;
    const rawAt = chunk.rawAt || 0;
    chunk.offset += 1;
    if (chunk.offset >= chunk.samples.length) queue.shift();
    return { value, rawAt };
  }

  function updateDelayEstimates(session) {
    if (!session.pendingDelayRawAt.size) return;
    const now = Date.now();
    for (const [streamName, rawAt] of session.pendingDelayRawAt) {
      const measuredDelay = clamp(now - rawAt, frameMs, maxDelayMs);
      const previousDelay = session.streamDelays.get(streamName) || 0;
      const nextDelay = previousDelay
        ? Math.round(previousDelay * 0.7 + measuredDelay * 0.3)
        : measuredDelay;
      session.streamDelays.set(streamName, nextDelay);
      session.pendingDelayRawAt.delete(streamName);
      session.syncStreams.delete(streamName);
    }
  }

  function requestSessionResync(session) {
    if (!session) return false;
    session.pendingDelayRawAt.clear();
    session.syncStreams = new Set(session.selectedStreams.map((stream) => stream.name));
    session.lastDelaySyncAt = Date.now();
    return true;
  }
  ...
  function getDelayMs(clientId, streamName) {
    const session = sessions.get(clientId);
    if (!session || !session.streamDelays.has(streamName)) return 0;
    return session.streamDelays.get(streamName) || 0;
  }

  function requestResync(clientId) {
    return requestSessionResync(sessions.get(clientId));
  }
