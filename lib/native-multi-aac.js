...
function createNativeMultiAac(options) {
  ...
  function createSession(clientId, selectedStreams, res) {
    ...
    const session = {
      ...
      gains,
      timelineMs: 0,
      streamLevels: new Map(selectedStreams.map((stream) => [stream.name, { timelineMs: 0, peak: 0 }])),
      ffmpeg,
      res,
      timer: null,
      backpressured: false,
  ...
  function writeMixedFrame(session) {
    ...
    const timelineMs = session.timelineMs;
    const framePeaks = new Map(session.selectedStreams.map((stream) => [stream.name, 0]));
    for (let frame = 0; frame < session.frameCount; frame += 1) {
      ...
      for (const stream of session.selectedStreams) {
        ...
        if (absValue > (framePeaks.get(stream.name) || 0)) framePeaks.set(stream.name, absValue);
    ...
    session.timelineMs += frameMs;
    for (const [streamName, peak] of framePeaks) {
      session.streamLevels.set(streamName, { timelineMs, peak: clamp(peak, 0, 1) });
    }
  }
  ...
  function getLevel(clientId, streamName) {
    const session = sessions.get(clientId);
    if (!session || !session.streamLevels.has(streamName)) return { timelineMs: 0, peak: 0 };
    return session.streamLevels.get(streamName);
  }
