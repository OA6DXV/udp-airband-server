...
function getSqliteRuntimeFiles(sqliteFile) {
  return [sqliteFile, sqliteFile + '-wal', sqliteFile + '-shm'];
}

function newestExistingFileMtime(files, fs) {
  let newest = 0;
  let newestFile = '';
  for (const filePath of files) {
  ...
  return { file: newestFile, mtime: newest };
}

function inspectLegacyJsonRuntimeStorage({ dataDir, fs, path, sqliteFile }) {
  const legacyFiles = findLegacyJsonFiles({ dataDir, fs, path });
  if (!legacyFiles.length) return { action: 'none', legacyFiles };

  const sqliteFiles = getSqliteRuntimeFiles(sqliteFile).filter((filePath) => fs.existsSync(filePath));
  if (!sqliteFiles.length) {
    return { action: 'migrate', legacyFiles, reason: 'sqlite_missing' };
  }

  const newestJson = newestExistingFileMtime(legacyFiles, fs);
  const newestSqlite = newestExistingFileMtime(sqliteFiles, fs);
  if (newestJson.mtime > newestSqlite.mtime) {
    return {
      action: 'migrate',
      legacyFiles,
      newestJsonFile: newestJson.file,
      newestSqliteFile: newestSqlite.file,
      reason: 'json_newer_than_sqlite',
    };
  }
  ...
  return {
    ...
    newestJsonFile: newestJson.file,
    newestSqliteFile: newestSqlite.file,
    reason: 'sqlite_current',
  };
}

function verifySqliteIntegrity(database) {
  const rows = database.db.prepare('PRAGMA integrity_check').all();
  const messages = rows
...
module.exports = {
  archiveLegacyJsonFiles,
  findLegacyJsonFiles,
  getLegacyJsonPaths,
  inspectLegacyJsonRuntimeStorage,
  migrateStorage,
  readJsonStorage,
