...
function createSqliteGeoCache({ database, logger }) {
  const { db, filePath } = database;
  const getStatement = db.prepare(`
    SELECT anonymized_ip, country_code, country, city, looked_up_at, source
    FROM geo_cache WHERE anonymized_ip = ?
  `);
  const entriesStatement = db.prepare(`
    SELECT anonymized_ip, country_code, country, city, looked_up_at, source
    FROM geo_cache ORDER BY anonymized_ip
  `);
  ...
  function get(anonymizedIp) {
    try {
      return mapRow(getStatement.get(anonymizedIp));
    } catch (err) {
  ...
  function entries() {
    try {
      return entriesStatement.all().map(mapRow);
    } catch (err) {
      logger.warn('geo_cache_read_failed', { path: filePath, error: err.message });
      return [];
    }
