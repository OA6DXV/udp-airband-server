...
port = 8584
enabled = true
# Set secure = true to enable Web Admin captcha protection.
# If key/cert are configured, Web Admin also serves HTTPS directly.
# If key/cert are empty, keep Web Admin behind an HTTPS reverse proxy so browser captcha can work.
# Set secure = false only for trusted private access; ALTCHA captcha is disabled.
secure = false
key =
cert =
...
[audio]
# Use the persistent AudioWorklet delivery path for individual raw and ADPCM streams.
# Unsupported browsers automatically fall back to the legacy packet scheduler.
worklet_streaming = true
# Browser jitter-buffer target and backlog ceiling for raw/ADPCM AudioWorklet playback.
worklet_target_latency_ms = 160
worklet_high_water_ms = 320
# Smooth UDP bursts by delivering raw PCM in 20 ms frames.
raw_pacing = true
...
codec = adpcm
adpcm_frame_ms = 20
# Smooth UDP bursts by delivering ADPCM frames at their media cadence.
adpcm_pacing = true
ffmpeg = ffmpeg
opus_bitrate = 24k
aac_bitrate = 32k
